						...
						if(vision.data_area>50)
						   aa = W_Coordinate.X;
							...
							if(Calculate_coordinate(aa,W_Coordinate.Y,W_Coordinate.Z)){
							 ...
							 one_servo.temp_angle = data2angel(125);
							 two_servo.temp_angle = data2angel(-125);
							 W_servo.temp_angle = data2angel(0);//-90
...
void Servo_Init(void) {
	                //舵机句柄    //最大角度     //最小角度       //零角度      //坐标系角度   //角度变大方向 //权重
		                //舵机句柄    //最大角度     //最小角度       //零角度      //坐标系角度   //角度变大方向 //权重
	Servo_motor_init(&one_servo , data2angel(140) , data2angel(0)   , data2angel(122+80) , data2angel(125)  , forward ,0.6f);
	Servo_motor_init(&two_servo , data2angel(120) , data2angel(-120) , data2angel(128) , data2angel(-125) , forward ,0.3f);
	Servo_motor_init(&W_servo   , data2angel(120) , data2angel(-120) , data2angel(115) , data2angel(0)   , back ,0.1f);
	Servo_motor_init(&bastic_servo ,data2angel(120) , data2angel(-120) , data2angel(-30) , data2angel(95)  , LEFT ,0.1f); //左正 右负
}
...
void SendCMD(uint8_t CMD,int time, uint8_t ID1,int angle1,uint8_t ID2,int angle2,uint8_t ID3,int angle3,uint8_t ID4,int angle4)
{
  ...
  data1=500+angle1*(2000/270);
	...
	HAL_UART_Transmit(&huart5,Tx_CMD,19,0xffff);
...
void Sendone(uint8_t CMD,int time, uint8_t ID1,int angle1)
{
  ...
  data1=500+angle1*(2000/270);
	...
	HAL_UART_Transmit(&huart5,Tx_CMD,10,0xffff);
